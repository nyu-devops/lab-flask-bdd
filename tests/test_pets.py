...
if not VCAP_SERVICES:
    VCAP_SERVICES = '{"rediscloud": [{"credentials": {' \
        '"password": "", "hostcustomer_name": "127.0.0.1", "port": "6379"}}]}'
...
class TestOrders(unittest.TestCase):
    ...
    def test_create_a_order(self):
        ...
        self.assertEqual(order.customer_name, "fido")
        self.assertEqual(order.order_time, "dog")
    ...
    def test_add_a_order(self):
        ...
        self.assertEqual(orders[0].customer_name, "fido")
        self.assertEqual(orders[0].order_time, "dog")
    ...
    def test_update_a_order(self):
        ...
        orders = Order.all()
        self.assertEqual(len(orders), 1)
        self.assertEqual(orders[0].order_time, "k9")
        self.assertEqual(orders[0].customer_name, "fido")
    ...
    def test_serialize_a_order(self):
        ...
        self.assertEqual(data['customer_name'], "fido")
        self.assertIn('order_time', data)
        self.assertEqual(data['order_time'], "dog")

    def test_deserialize_a_order(self):
        """ Deserialize a Order """
        data = {"id":1, "customer_name": "kitty", "order_time": "cat", "status": True}
        order = Order(data['id'])
        order.deserialize(data)
        self.assertNotEqual(order, None)
        self.assertEqual(order.id, 1)
        self.assertEqual(order.customer_name, "kitty")
        self.assertEqual(order.order_time, "cat")

    def test_deserialize_with_no_customer_name(self):
        """ Deserialize a Order that has no customer_name """
        data = {"id":0, "order_time": "cat"}
        order = Order(0)
    ...
    def test_save_a_order_with_no_customer_name(self):
        """ Save a Order with no customer_name """
    ...
    def test_find_by_customer_name(self):
        ...
        Order(0, "fido", "dog").save()
        Order(0, "kitty", "cat").save()
        orders = Order.find_by_customer_name("fido")
        self.assertNotEqual(len(orders), 0)
        self.assertEqual(orders[0].order_time, "dog")
        self.assertEqual(orders[0].customer_name, "fido")

    def test_find_by_order_time(self):
        """ Find a Order by Category """
        Order(0, "fido", "dog").save()
        Order(0, "kitty", "cat").save()
        orders = Order.find_by_order_time("cat")
        self.assertNotEqual(len(orders), 0)
        self.assertEqual(orders[0].order_time, "cat")
        self.assertEqual(orders[0].customer_name, "kitty")
