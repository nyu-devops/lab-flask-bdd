...
for a server customer_name 'redis' to connect to.
"""
...
class Order(object):
    ...
    logger = logging.getLogger(__customer_name__)
    redis = None
    schema = {
        'id': {'type': 'integer'},
    ...
    def __init__(self, id=0, customer_name=None, order_time=None, status=True):
        """ Constructor """
        self.id = int(id)
    ...
    def save(self):
        ...
        if self.customer_name is None:   # customer_name is the only required field
            raise DataValidationError('customer_name attribute is not set')
        if self.id == 0:
...
######################################################################
    ...
    def __find_by(attribute, value):
        ...
        # return [order for order in Order.__data if order.order_time == order_time]
    ...
    def find_by_customer_name(customer_name):
        """ Query that finds Orders by their customer_name """
        return Order.__find_by('customer_name', customer_name)

    @staticmethod
    def find_by_order_time(order_time):
        """ Query that finds Orders by their order_time """
        return Order.__find_by('order_time', order_time)
...
######################################################################
    ...
    @staticmethod
    def connect_to_redis(hostcustomer_name, port, password):
        """ Connects to Redis and tests the connection """
        Order.logger.info("Testing Connection to: %s:%s", hostcustomer_name, port)
        Order.redis = Redis(host=hostcustomer_name, port=port, password=password)
        try:
            Order.redis.ping()
            Order.logger.info("Connection established")
        except ConnectionError:
            Order.logger.info("Connection Error from: %s:%s", hostcustomer_name, port)
            Order.redis = None
        return Order.redis
    ...
    def init_db(redis=None):
        ...
        if 'VCAP_SERVICES' in os.environ:
            ...
            Order.logger.info("Conecting to Redis on host %s port %s",
                            creds['hostcustomer_name'], creds['port'])
            Order.connect_to_redis(creds['hostcustomer_name'], creds['port'], creds['password'])
        else:
